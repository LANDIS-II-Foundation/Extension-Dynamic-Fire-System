    ...
    public class EventRegion
    {
        ...
        private static double CalculateTravelTime(Site site, Site firesource, Event fireEvent, bool buildUp)
        {
            ...
            else
            {
                ...
                double dist = CellDistanceBetweenSites(site, firesource) * Model.Core.CellLength;  //dist = r in Calvert's equation
                double r = ROSi;
                double time = 0;
                double index = 1;
                ...
                if (site == firesource)
                {
                    ...
                    double cost = rate * Model.Core.CellLength / 2.0;     //units = minutes
                    return cost;
                }
                else
                {
                    ...
                    //if (cost < 1)
                     //   UI.WriteLine("Travel time < 1 min");
                    return cost;
                }
            }
        }
        ...
        private static bool ElipseDistanceBetweenSites(Site igSite, Site burnSite, double C, double A, double fireDirection, double ellipseArea, SizeType fireSizeType)
        {
            ...
            C = C / (double)Model.Core.CellLength;
            A = A / (double)Model.Core.CellLength;
            ...
            double radius = Math.Sqrt((ellipseArea / Math.PI)) / (double)Model.Core.CellLength;
